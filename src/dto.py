...
class Payment:
    ...
    def safe(self) -> str:
        if self.name.lower() == 'счет':
            ...
            safe_number = self._split_card_number_by_blocks(safe_number)
    ...
    @staticmethod
    def _split_card_number_by_blocks(card_number: str) -> str:
        block_sizes = (4, 4, 4, 4)
        result = []
    ...
    def init_from_dict(cls, data):
        return cls(
            ...
            amount=Amount(
                ...
                currency_code=data['operationAmount']['currency']['name']
            ),
